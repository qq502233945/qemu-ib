			 ...
			 struct fuse_bufvec *ibufv)
{
	...
	if (fuse_buf_size(pbufv) != arg->size) {
		fprintf(stderr, "fuse: do_write_buf: buffer size doesn't match arg->size\n");
...
static char* find_path_by_ino(char* path, ino_t ino) 
{
    DIR* dp;
    struct dirent* dirp;
    char* sub_path = NULL;
				  ...
				  struct fuse_bufvec *bufv, struct fuse_chan *ch)
{
	...
	if (se->deny_others && in->uid != se->owner && in->uid != 0 &&
		 ...
		 in->opcode != FUSE_READDIRPLUS && in->opcode != FUSE_IB_READ)
		goto reply_err;
